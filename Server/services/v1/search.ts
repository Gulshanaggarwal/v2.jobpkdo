...
const funcs = {
	searchJob: async (search: string, filter: string, next_token: string) => {
		try {
			const tweetQuery = `("we are hiring" OR "is hiring" OR "we are looking for" OR "we are looking for") ${search} ${filter} -flatmate -roommate -something -sex lang:en -is:retweet`;
